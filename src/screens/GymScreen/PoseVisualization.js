...
const PoseVisualization = ({ keypoints, cameraType }) => {
  console.log(
    'Posevisualization called with: ',
    keypoints ? `${keypoints.points?.length || 0} points` : 'no keypoints',
    cameraType,
  );
  ...
  const COLORS = {
    torso: 'rgba(0, 255, 255, 0.8)',
    leftArm: 'rgba(0, 255, 0, 0.8)',
    rightArm: 'rgba(255, 255, 0, 0.8)',
    leftLeg: 'rgba(255, 0, 255, 0.8)',
    rightLeg: 'rgba(255, 165, 0, 0.8)',
  };
  ...
  const getConnectionColor = (from, to) => {
    ...
    if (
      (from === mp_pose.LEFT_SHOULDER && to === mp_pose.RIGHT_SHOULDER) ||
    ...
    if (
      (from === mp_pose.LEFT_SHOULDER && to === mp_pose.LEFT_ELBOW) ||
    ...
    if (
      (from === mp_pose.RIGHT_SHOULDER && to === mp_pose.RIGHT_ELBOW) ||
    ...
    if (
      (from === mp_pose.LEFT_HIP && to === mp_pose.LEFT_KNEE) ||
    ...
    if (
      (from === mp_pose.RIGHT_HIP && to === mp_pose.RIGHT_KNEE) ||
